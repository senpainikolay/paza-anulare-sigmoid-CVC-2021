...
class blackboard2:
    ...
    def draw(self, canvas):
        ...
        if self.state == States[0]:
            ...
            if self.mouse.mouseMode == 'Draw' and self.last_mode == self.mouse.mouseMode:
                canvas = cv2.line(canvas, self.last_coord, self.coord, (0,0,0), self.grosime*5)
            elif self.mouse.mouseMode == 'Draw' and self.last_mode != self.mouse.mouseMode:
                canvas = cv2.line(canvas, (self.coord[0]-5, self.coord[1]-5), self.coord, (0,0,0), self.grosime*5) 
    ...
    def start(self,frame ):
        ...
        self.board  = self.draw(self.board) 
        self.board, self.shapeBoard = self.shapes(self.board)
        self.drawMenu()    
        frame = self.converterCanvas(frame,self.board)
        frame = self.converterCanvas(frame,self.shapeBoard) 
        frame = self.converterCanvas(frame,self.menuBoard)
        return frame